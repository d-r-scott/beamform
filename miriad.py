...
class MiriadGainSolutions(object):
    ...
    def __init__(self, file_root, bp_c_root=None, pol=None, freqs=None):
        ...
        if bp_c_root == None and file_root == None:
            ...
            self.bp_real_interp = [
                interp1d(self.freqs, bp_real[:, iant], fill_value=(self.bp_real[0, iant], self.bp_real[-1, iant]),
                         bounds_error=False) for iant in xrange(nant)]
            self.bp_imag_interp = [
                interp1d(self.freqs, bp_imag[:, iant], fill_value=(self.bp_imag[0, iant], self.bp_imag[-1, iant]),
                         bounds_error=False) for iant in xrange(nant)]
            self.bp_coeff = None
        else:
            ...
            if "polyfit_coeff" in bp_c_root:  # AIPS polyfit coefficient
                ...
                bw = len(freqs)  # in MHz
                self.freqs = (-np.arange(float(nfreq)) / nfreq * bw + fmax - float(
                    bw) / nfreq / 2) / 1e3  # reassign freqs in GHz
                ...
                if len(readme) == 1:
                    ...
                    fring_f = bp_c_root.replace(bp_c_root.split('/')[-1],
                                                "delays.sn.txt")  # ("bandpasses.bp.txt","delays.sn.txt")
                ...
                if '190608' in bp_c_root:
                    specific_frb = '190608'
                aips_cor = aipscor(fring_f, sc_f, bp_c_root, specific_frb=specific_frb)
                for iant in range(nant):
                    ...
                    phases -= phases[
                        int(len(phases) / 2)]  # TODO! READ THE REFERENCE FREQUENCY AND SET TO THAT REFERENCE
                    # print(np.shape(phases),np.shape(bp))
                    bp *= np.exp(np.pi * 2j * phases, dtype=np.complex64)
                    if 0:  # temp
                        print('Polarization fring file exists, also applying this delay')
                        if pol == 'y':
                            phase_offset = -9.79084693013  # 6.66130639185 #10.2025399576 #10.347862410779019#(offsetlarger)# temp, got from VELA
                ...
                self.bp_real_interp = [interp1d(self.freqs, self.bp_real[:, iant],
                                                fill_value=(self.bp_real[0, iant], self.bp_real[-1, iant]),
                                                bounds_error=False) for iant in xrange(nant)]
                self.bp_imag_interp = [interp1d(self.freqs, self.bp_imag[:, iant],
